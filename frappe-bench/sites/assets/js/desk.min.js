...
frappe.ui.notifications = {
	...
	update_notifications: function update_notifications() {
		...
		$(".navbar-new-comments").html(this.total > 20 ? '20+' : this.total).toggleClass("navbar-new-comments-true", this.total ? true : false).parent().toggleClass("disabled", this.total ? false : true);
	},
...
frappe.ui.Graph = function () {
	...
	_createClass(Graph, [{
		...
		value: function setup_container() {
			...
			this.container = $('<div>').addClass('graph-container').append($('<h6 class="title" style="margin-top: 15px;">' + this.title + '</h6>')).append($('<h6 class="sub-title uppercase">' + this.subtitle + '</h6>')).append($('<div class="graph-graphics"></div>')).append($('<div class="graph-stats-container"></div>')).appendTo(this.parent);
		...
		value: function calc_min_tops() {
			...
			this.y.map(function (d) {
				d.y_tops.map(function (y_top, i) {
					if (y_top < _this3.y_min_tops[i]) {
						_this3.y_min_tops[i] = y_top;
					}
				});
			});
		}
		...
		value: function make_graph_components() {
			...
			this.y.map(function (d, i) {
				_this4.make_units(d.y_tops, d.color, i);
				_this4.make_path(d);
			});
		...
		value: function make_y_axis() {
			...
			this.y_axis_values.map(function (point) {
				_this5.y_axis_group.add(_this5.snap.g(_this5.snap.line(start_at, 0, width, 0), _this5.snap.text(text_end_at, 0, point + "").attr({
					dy: ".32em",
					class: "y-value-text"
				})).attr({
					class: 'tick ' + label_class,
					transform: 'translate(0, ' + (_this5.height - point * _this5.multiplier) + ')'
				}));
			});
		}
	}, {
		...
		value: function make_x_axis() {
			...
			this.x.values.map(function (point, i) {
				_this6.x_axis_group.add(_this6.snap.g(_this6.snap.line(0, 0, 0, height), _this6.snap.text(0, text_start_at, point).attr({
					dy: ".71em",
					class: "x-value-text"
				})).attr({
					class: 'tick ' + label_class,
					transform: 'translate(' + _this6.x_axis_values[i] + ', 0)'
				}));
			});
		}
	}, {
		...
		value: function make_units(y_values, color, dataset_index) {
			...
			y_values.map(function (y, i) {
				var data_unit = _this7.draw[d.type](_this7.x_axis_values[i], y, d.args, color, dataset_index);
				_this7.data_units.add(data_unit);
				_this7.y[dataset_index].data_units.push(data_unit);
			});
		}
	}, {
		key: 'make_path',
		...
		value: function bind_tooltip() {
			...
			this.$graphics.on('mousemove', function (e) {
				var offset = $(_this8.$graphics).offset();
				var relX = e.pageX - offset.left - _this8.translate_x;
				var relY = e.pageY - offset.top - _this8.translate_y;

				if (relY < _this8.height) {
					for (var i = _this8.x_axis_values.length - 1; i >= 0; i--) {
						var x_val = _this8.x_axis_values[i];
						if (relX > x_val - _this8.avg_unit_width / 2) {
							var x = x_val - _this8.tip.width() / 2 + _this8.translate_x;
							var y = _this8.y_min_tops[i] - _this8.tip.height() + _this8.translate_y;

							_this8.fill_tooltip(i);
			...
			this.$graphics.on('mouseleave', function () {
				_this8.tip.attr({
					style: 'top: 0px; left: 0px; opacity: 0; pointer-events: none;'
				});
			});
		...
		key: 'fill_tooltip',
		value: function fill_tooltip(i) {
			var _this9 = this;

			this.tip_title.html(this.x.formatted && this.x.formatted.length > 0 ? this.x.formatted[i] : this.x.values[i]);
			this.tip_data_point_list.empty();
			this.y.map(function (y_set) {
				var $li = $('<li>\n\t\t\t\t<strong style="display: block;">\n\t\t\t\t\t' + (y_set.formatted ? y_set.formatted[i] : y_set.values[i]) + '\n\t\t\t\t</strong>\n\t\t\t\t' + (y_set.title ? y_set.title : '') + '\n\t\t\t</li>').addClass('border-top ' + y_set.color);
				_this9.tip_data_point_list.append($li);
			});
		}
	}, {
		...
		value: function show_specific_values() {
			...
			this.specific_values.map(function (d) {
				_this10.specific_y_lines.add(_this10.snap.g(_this10.snap.line(0, 0, _this10.width, 0).attr({
					class: d.line_type === "dashed" ? "graph-dashed" : ""
				}), _this10.snap.text(_this10.width + 5, 0, d.name.toUpperCase()).attr({
					dy: ".32em",
					class: "specific-value"
				})).attr({
					class: "tick",
					transform: 'translate(0, ' + (_this10.height - d.value * _this10.multiplier) + ')'
				}));
			});
		}
	}, {
		...
		value: function change_values(new_y) {
			...
			this.y.map(function (d, i) {
				...
				new_d.y_tops = new_d.values.map(function (val) {
					return _this12.height - val * _this12.multiplier;
				});

				d.data_units.map(function (unit, j) {
					var current_y_top = d.y_tops[j];
					var current_height = _this12.height - current_y_top;

					var new_y_top = new_d.y_tops[j];
					var new_height = current_height - (new_y_top - current_y_top);

					_this12.animate[u.type](unit, new_y_top, { new_height: new_height });
				});
			});
			...
			if (this.y[0].path) {
				new_y.map(function (e, i) {
					var new_points_list = e.y_tops.map(function (y, i) {
						return _this12.x_axis_values[i] + ',' + y;
					});
					var new_path_str = "M" + new_points_list.join("L");
					_this12.y[i].path.animate({ d: new_path_str }, 300, mina.easein);
				});
			}
		}
	}, {
		...
		value: function setup_utils() {
			...
			this.draw = {
				'bar': function bar(x, y, args, color, index) {
					var total_width = _this13.avg_unit_width - args.space_width;
					var start_x = x - total_width / 2;

					var width = total_width / args.no_of_datasets;
					var current_x = start_x + width * index;
					return _this13.snap.rect(current_x, y, width, _this13.height - y).attr({
						class: 'bar mini fill ' + color
					});
				},
				'dot': function dot(x, y, args, color) {
					return _this13.snap.circle(x, y, args.radius).attr({
						class: 'fill ' + color
					});
				}
			};
...
frappe.ui.BarGraph = function (_frappe$ui$Graph) {
	...
	function BarGraph() {
		...
		return _possibleConstructorReturn(this, (BarGraph.__proto__ || Object.getPrototypeOf(BarGraph)).call(this, args));
	}
	...
	_createClass(BarGraph, [{
		...
		value: function setup_values() {
			...
			this.unit_args = {
				type: 'bar',
				args: {
					space_width: this.y.length > 1 ? me.avg_unit_width / 2 : me.avg_unit_width / 8,
...
frappe.ui.LineGraph = function (_frappe$ui$Graph2) {
	...
	function LineGraph() {
		...
		return _possibleConstructorReturn(this, (LineGraph.__proto__ || Object.getPrototypeOf(LineGraph)).call(this, args));
	}
	...
	_createClass(LineGraph, [{
		...
		key: 'make_path',
		value: function make_path(d) {
			var _this16 = this;

			var points_list = d.y_tops.map(function (y, i) {
				return _this16.x_axis_values[i] + ',' + y;
			});
...
frappe.ui.PercentageGraph = function (_frappe$ui$Graph3) {
	...
	function PercentageGraph() {
		...
		return _possibleConstructorReturn(this, (PercentageGraph.__proto__ || Object.getPrototypeOf(PercentageGraph)).call(this, args));
	}
	...
	_createClass(PercentageGraph, [{
		...
		value: function make_graph_components() {
			...
			var grand_total = this.x.totals.reduce(function (a, b) {
				return a + b;
			}, 0);
			this.x.units = [];
			this.x.totals.map(function (total, i) {
				var $part = $('<div class="progress-bar background ' + _this19.x.colors[i] + '"\n\t\t\t\tstyle="width: ' + total * 100 / grand_total + '%"></div>');
				_this19.x.units.push($part);
				_this19.$percentage_bar.append($part);
			});
		}
	}, {
		key: 'make_tooltip',
		...
		value: function show_summary() {
			...
			var values = this.x.formatted.length > 0 ? this.x.formatted : this.x.values;
			this.x.totals.map(function (d, i) {
				_this20.$stats_container.append($('<div class="stats">\n\t\t\t\t<span class="indicator ' + _this20.x.colors[i] + '">\n\t\t\t\t\t<span class="text-muted">' + values[i] + ':</span>\n\t\t\t\t\t' + d + '\n\t\t\t\t</span>\n\t\t\t</div>'));
			});
		}
	}]);
